...
`--emit-archives DIR` packs each ecosystem directory into a byte-reproducible
`DIR/<ecosystem>.tar.gz` and writes `DIR/archive-plan.json` beside them, recording each ecosystem's
content digest, the release tag that digest implies, the asset name, and whether it differs from the
digest pinned in `ecosystem-explorer/public/data-manifest.json`. That digest comparison is what lets
the workflow skip republishing unchanged data; the archive bytes play no part in it. The archives
are byte-reproducible for a different reason, so that rebuilding the same registry state does not
churn an already published asset.
