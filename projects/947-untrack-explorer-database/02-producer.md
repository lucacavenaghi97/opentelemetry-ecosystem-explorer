...
4. For each ecosystem whose digest changed, and only for the ecosystems being promoted:
   ...
   3. `gh release create <tag> --draft --latest=false --target <sha> --title "<ecosystem> data <date>"`.
   4. `gh release upload <tag> <ecosystem>.tar.gz`.
   5. `gh release edit <tag> --draft=false --latest=false`.
...
| 8   | Rehearse the release path on a fork                             | Two dispatches, one release, bytes verified from the anonymous URL       |
...
- **`--latest=false` did not keep the Latest badge away.** That is why the releases are now created
  as pre-releases. This is the one defect the rehearsal found.
